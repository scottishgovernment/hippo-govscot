...
/**
 * Implements operations we want to perform with Funnelback: indexing content and maintaining the journal position.
 * Connection settings are read from {@link FunnelbackConfiguration}; site-specific journal position
 * values are read from {@link FunnelbackIndexingConfiguration}.
 */
public class FunnelbackIndexerImpl implements FunnelbackIndexer {

    private static final Logger LOG = LoggerFactory.getLogger(FunnelbackIndexerImpl.class);

    private static final String SECURITY_TOKEN = "X-Security-Token";

    private static final String CHARSET = "UTF-8";

    /** Funnelback filter providers applied to indexed content; fixed across all environments. */
    private static final String FILTERS = "TikaFilterProvider,ExternalFilterProvider:JSoupProcessingFilterProvider:DocumentFixerFilterProvider";
    ...
    public FunnelbackIndexerImpl(FunnelbackConfiguration configuration, FunnelbackIndexingConfiguration indexingConfiguration) {
        this.configuration = configuration;
        funnelBackUri = URI.create(configuration.apiUrl());
        funnelbackHost = new HttpHost(funnelBackUri.getHost(), funnelBackUri.getPort(), funnelBackUri.getScheme());
        this.positionUrl = funnelbackCollectionUrl(
                indexingConfiguration.positionCollection(),
                indexingConfiguration.positionKey());
    ...
    public void unpublish(String collection, String key) throws FunnelbackException {
        String url = funnelbackCollectionUrl(collection, key);
        try {
    ...
    String funnelbackCollectionUrl(String collection, String key) {
        try {
            ...
            String url = String.format(
                    ...
                    encodedKey,
                    encodedFilters);
            LOG.debug("FunnelbackCollectionUrl {}", url);
            return url;
    ...
    int execute(HttpRequestBase request) throws IOException {
        ...
        stopWatch.start();
        request.addHeader(SECURITY_TOKEN, configuration.apiKey());
        ...
        try {
            ...
            stopWatch.stop();
            LOG.debug("Request {} {} execute took {}", request.getMethod(), request.getURI(), stopWatch.getTime());
            response.close();
        }
    }
    ...
    void storeJournalPosition(String url, JournalPosition position) throws IOException, FunnelbackException {
        ...
        request.setHeader("X-Journal-Position", positionString);
        CloseableHttpResponse response = httpClient.execute(funnelbackHost, request);
        try {
            if (response.getStatusLine().getStatusCode() != 200) {
