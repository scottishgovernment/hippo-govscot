...
/**
 * Wrap a Funnelback object in order to collect metrics.
 */
public class MetricsCollectingFunnelbackImpl implements FunnelbackIndexer {
    ...
    MetricsCollectingFunnelbackImpl(FunnelbackIndexer funnelback) {
    ...
    public void unpublish(String collection, String key) throws FunnelbackException {
        try {
            ...
            requestCounter.inc();
            requestMeter.mark();
            funnelback.unpublish(collection, key);
            timer.stop();
        } catch (FunnelbackException e) {
            errorCounter.inc();
            errorMeter.mark();
