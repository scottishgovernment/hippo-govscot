 ...
 * so different sites or search engines can plug in their own implementationx
...
public class JournalReconciliationLoop implements RepositoryJob {
    ...
    void doExecute(RepositoryJobExecutionContext context, Session session, FeatureFlag featureFlag) throws RepositoryException {
        ...
        try (JournalConsumer journalConsumer = journalConsumerFactory.newConsumer(context, session)){
            ...
            LOG.error("JournalConsumerFactory could not create a journal consumer or position source, skipping", e);
        } catch (Exception e) {
            LOG.error("Exception in doExcecute", e);
        }
    }
