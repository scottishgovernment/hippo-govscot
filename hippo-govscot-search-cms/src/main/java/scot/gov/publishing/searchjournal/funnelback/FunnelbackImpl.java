...
public class FunnelbackImpl implements Funnelback {
    ...
    String funnelbackCollectionUrl(String collection, String key) {
        try {
            ...
            // strip the account prefix if it is already present in the collection name
            if (collection.startsWith(accountName + "~")) {
                collection = StringUtils.substringAfter(collection, accountName + "~");
            }

            String url;
            LOG.debug("funnelbackCollectionUrl collection {}, searchType {}", collection, configuration.getSearchType());
