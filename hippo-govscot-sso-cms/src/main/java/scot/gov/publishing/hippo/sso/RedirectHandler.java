...
/**
 * Decides whether a request must be sent to the IdP to authenticate, or can pass through to
 * the CMS webapp, and issues the redirect when required. Called directly by {@code SsoFilter} —
 * not itself a {@code Filter}, since {@code SsoFilter} is the only one declared in web.xml.
 */
public class RedirectHandler {
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        LOG.debug("RedirectHandler - {} {}", request.getMethod(), requestUrl);
